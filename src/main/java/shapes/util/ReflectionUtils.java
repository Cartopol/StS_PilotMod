...
public class ReflectionUtils {
    ...
    public static <T> ArrayList<Class<T>> findAllConcreteShapesModClasses(ClassFilter classFilter)
    {
        try {
            ClassFinder finder = new ClassFinder();
            URL url = ShapesMod.class.getProtectionDomain().getCodeSource().getLocation();
    ...
    public static <T> ArrayList<Class<T>> findAllConcreteShapesModSubclasses(Class<T> baseJorbsModClass) {
        return findAllConcreteShapesModClasses(new SubclassClassFilter(baseJorbsModClass));
    }

    // Note: only works for base classes defined within our own jar (limitation of ClassFinder)
    // Requires each class have a no-arg constructor
    public static <T> List<T> instantiateAllConcreteShapesModSubclasses(Class<T> baseWarlordModClass) {
        ArrayList<Class<T>> classes = findAllConcreteShapesModClasses(new SubclassClassFilter(baseWarlordModClass));
