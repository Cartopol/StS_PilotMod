...
import shapes.cards.DowngradeableCard;
...
public class CardMetaUtils {
    ...
    public static void downgradeCardPermanently(AbstractCard card) {
        if (!card.purgeOnUse && (card instanceof DowngradeableCard) && card.upgraded) {
            ShapesMod.logger.info("Downgrading " + card.toString());
            ...
            if (masterCard != null) {
                ...
                ShapesMod.logger.info("Card to downgrade is not in the deck: " + card.cardID);
            }
        }
    }
    ...
    public static void destroyCardPermanently(AbstractCard card) {
        ...
        AbstractCard masterCard = StSLib.getMasterDeckEquivalent(card);
        if (masterCard != null) {
            ShapesMod.logger.info("Removing " + masterCard.toString());
            AbstractDungeon.player.masterDeck.removeCard(masterCard);

        } else {
            ShapesMod.logger.info("Failed to purge a card we didn't have. Perhaps Duplication Potion or Attack Potion or similar effect occurred. " + card.cardID);
        }
    }
