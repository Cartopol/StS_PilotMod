...
// For use with cards like "Gain !B! Block (!shapes:BaseBlock! + !M! block per something)".
// Should generally only be used *in addition to* !B!, not *instead of* !B!.
public class BaseBlockNumber extends DynamicVariable {
    // Reference as "!shapes:BaseBlock!" in card strings to actually display the number.
    @Override
    public String key() {
