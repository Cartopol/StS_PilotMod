...
public class ReflectionUtils {
    ...
    public static <T> ArrayList<Class<T>> findAllConcretePilotModClasses(ClassFilter classFilter)
    {
        try {
            ClassFinder finder = new ClassFinder();
            URL url = PilotMod.class.getProtectionDomain().getCodeSource().getLocation();
    ...
    public static <T> ArrayList<Class<T>> findAllConcretePilotModSubclasses(Class<T> baseJorbsModClass) {
        return findAllConcretePilotModClasses(new SubclassClassFilter(baseJorbsModClass));
    }

    // Note: only works for base classes defined within our own jar (limitation of ClassFinder)
    // Requires each class have a no-arg constructor
    public static <T> List<T> instantiateAllConcretePilotModSubclasses(Class<T> basePilotModClass) {
        ArrayList<Class<T>> classes = findAllConcretePilotModClasses(new SubclassClassFilter(basePilotModClass));
