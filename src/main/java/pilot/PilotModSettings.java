...
public class PilotModSettings {
    private static Properties DEFAULT_SETTINGS = new Properties();
    static {
    }

    private static final String MOD_SETTINGS_FILE = "stspilotmod_config";
    ...
    public static void initialize() {
        try {
            config = new SpireConfig(PilotMod.MODNAME, MOD_SETTINGS_FILE, DEFAULT_SETTINGS);
            config.load();
        } catch (Exception e) {
            e.printStackTrace();
        }
